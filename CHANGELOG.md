...
### Added
- **Mid-turn steering.** A message sent while Claude is working now interrupts and redirects it — the agent waits ~1.5 seconds (debounced, so a burst of follow-ups collapses into one interrupt), then gracefully aborts the current turn and processes the new message next. The session and queue are preserved; no respawn. Opt out with `cta config interrupt-steer off` (or the Pager "Mid-turn steering" toggle). Default: on.
- **`/stop` is now graceful.** Sending `/stop` mid-turn interrupts Claude in-band (session preserved, same daemon) instead of killing and respawning the subprocess. Your next message picks up the conversation immediately. Falls back to the old kill path if the daemon's stdin is unavailable.
- **The bot now publishes a `/` command menu.** Type `/` in Telegram and the common commands (`/stop`, `/clear`, `/cost`, `/list`, `/mount`, …) autocomplete with descriptions — no more remembering them. Registered once on poller startup via `setMyCommands`.
- **Idle conversations that grow large now reset themselves — and remember the important bits first.** The idle setting (Pager "Idle sessions", `cta config idle-evict`) used to only free memory: a quiet topic's session closed but resumed the same (possibly huge, expensive) conversation on your next message. Now, when a quiet topic's transcript has grown large (>1 MB), the bot first saves the durable context (your preferences, ongoing tasks, open decisions, key facts) to its cross-session memory, then starts a fresh session — so replies stay fast and cheap without forgetting what matters. Small conversations still just free memory with no context loss. Tunable via `IDLE_CLEAR_MIN_BYTES` / `IDLE_FLUSH_TIMEOUT_MS`.
